...
class SAC_META(object):
    def __init__(self, num_inputs, action_space, config):
        ...
        self.log_alpha_max = float(config['log_alpha_max'])
        self.log_alpha_min = float(config.get('log_alpha_min', -4.0))
    ...
    def update_parameters(self, memory, kl_memory, batch_size, updates, s0_list=None):
        ...
        if updates % self.meta_train_interval == 0:
            ...
            # ``backward(create_graph=True)`` creates a parameter/gradient
            # reference cycle in PyTorch.  The simulated policy update above no
            # longer needs the policy gradients after the meta-gradient has been
            # applied, so clear them explicitly to reduce long-notebook OOM
            # crashes during many-episode solar-tracker training runs.
